...
# app/main.py (fix: botocore.config + DB-Retry)
@"
...
def wait_db(max_tries=30, sleep_s=1.0):
    ...
    for _ in range(max_tries):
        ...
        except Exception as e: last_err=e; time.sleep(sleep_s)
    raise last_err
...
# Override (erzwingt Uvicorn; überschreibt Node)
@"
services:
  api:
    build:
...
# Health warten
$ok=$false; foreach($i in 1..40){ try{ $r=Invoke-RestMethod http://localhost:8080/health -TimeoutSec 2; if($r.status -eq "ok"){ $ok=$true; break } } catch{}; Start-Sleep -Milliseconds 750 }
if(-not $ok){
...
Write-Host "==> Redeploy + Smoke: fertig" -ForegroundColor Green
