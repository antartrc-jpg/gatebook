...
s3_int=boto3.client("s3", endpoint_url=S3_ENDPOINT, aws_access_key_id=S3_ACCESS_KEY, aws_secret_access_key=S3_SECRET_KEY, config=Config(s3={"addressing_style":"path"}, signature_version="s3v4"), region_name="us-east-1")
s3_pub=boto3.client("s3", endpoint_url=PUBLIC_S3_ENDPOINT, aws_access_key_id=S3_ACCESS_KEY, aws_secret_access_key=S3_SECRET_KEY, config=Config(s3={"addressing_style":"path"}, signature_version="s3v4"), region_name="us-east-1")

class PreSignIn(BaseModel):
    filename: str; content_type: str; size_bytes: int = Field(ge=1, le=1024*1024*1024)
class PreSignOut(BaseModel):
    file_id: str; s3_key: str; url: str
class ConfirmIn(BaseModel):
    file_id: str; sha256_hex: str
...
def files_presign(inp:PreSignIn):
    ...
    url=s3_pub.generate_presigned_url("put_object", Params={"Bucket":S3_BUCKET,"Key":s3_key,"ContentType":inp.content_type}, ExpiresIn=900)
    conn=db(); cur=conn.cursor()
    cur.execute("insert into file_object(id, filename, content_type, s3_key, within_24h) values (%s,%s,%s,%s,true)", (file_id, inp.filename, inp.content_type, s3_key))
    cur.execute("insert into audit_log(action, entity, entity_id, meta) values (%s,%s,%s,%s)", ("files.presign","file_object",file_id, psycopg2.extras.Json({"filename":inp.filename})))
    conn.commit(); cur.close(); conn.close()
...
def files_confirm(inp:ConfirmIn):
    ...
    key=row[0]; head=s3_int.head_object(Bucket=S3_BUCKET, Key=key)
    size_bytes=head.get("ContentLength"); content_type=head.get("ContentType","application/octet-stream")
    conn=db(); cur=conn.cursor()
    cur.execute("update file_object set size_bytes=%s, content_type=%s, sha256_hex=%s, server_received_at=now() where id=%s",(size_bytes,content_type,inp.sha256_hex,inp.file_id))
    cur.execute("insert into audit_log(action, entity, entity_id, meta) values (%s,%s,%s,%s)", ("files.confirm","file_object",inp.file_id, psycopg2.extras.Json({"size":size_bytes})))
    conn.commit(); cur.close(); conn.close()
    return {"ok":True,"size_bytes":size_bytes,"content_type":content_type}
"@ | Set-Content -Path (Join-Path $APP "main.py") -Encoding UTF8

# 1) Image separat bauen (unabhängig vom Compose-Build)
docker build -t gatebook-fastapi:dev -f "$($API)\Dockerfile.fastapi" "$API"

# 2) Override: zwinge unser Image + Uvicorn
@"
services:
  api:
    image: gatebook-fastapi:dev
    command: ["uvicorn","app.main:app","--host","0.0.0.0","--port","8080"]
    env_file:
      - ../api/.env
"@ | Set-Content -Path (Join-Path $INFRA "docker-compose.override.yml") -Encoding UTF8

# 3) Alt stoppen/entfernen (Fehlerausgabe unterdrücken, unkritisch)
docker compose -f .\docker-compose.dev.yml -f .\docker-compose.override.yml stop api 2>$null
docker compose -f .\docker-compose.dev.yml -f .\docker-compose.override.yml rm -sf api 2>$null

# 4) Neu starten – erzwinge Recreate ohne Dependencies
docker compose -f .\docker-compose.dev.yml -f .\docker-compose.override.yml up -d --force-recreate --no-deps api
...
# 6) Smoke: Presign -> PUT -> Confirm -> DB-View
...
Write-Host "==> Switch + Smoke: fertig" -ForegroundColor Green
