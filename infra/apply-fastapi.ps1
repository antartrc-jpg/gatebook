...
Write-Host "==> APPLY FASTAPI + REDEPLOY + SMOKE" -ForegroundColor Cyan
...
def health_full():
    ...
    except Exception as e: checks["s3"]="err:"+str(e); status="degraded"
    try:
        s=socket.create_connection(("redis",6379),1); s.close(); checks["redis"]="ok"
    except Exception as e: checks["redis"]="err:"+str(e); status="degraded"
...
# --- Override: Build auf Dockerfile.fastapi + Volumes neutralisieren
@"
services:
  api:
    build:
...
Write-Host "==> APPLY FASTAPI: fertig" -ForegroundColor Green
